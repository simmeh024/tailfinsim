...
describe('one-shot paid boundary with a real durable ledger and fake provider', () => {
  ...
  it('requires explicit prepare/submit selectors and rejects paid retries or root overrides', () => {
    ...
    expect(parseMeshyRunArguments(['retexture-provenance', '--max-credits', '40']).command).toBe(
      'retexture-provenance',
    );
    expect(
      parseMeshyRunArguments(['archive-retexture-init', '--approval-file', 'private.json']).command,
    ).toBe('archive-retexture-init');
    expect(
    ...
    expect(
      parseMeshyRunArguments(['archive-retexture-sync', '--max-total-credits', '50']).command,
    ).toBe('archive-retexture-sync');
    for (const args of [
      ['prepare'],
      ...
      ['archive-retexture-sync', '--max-total-credits', '49'],
