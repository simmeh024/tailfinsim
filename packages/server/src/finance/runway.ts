...
function classifyCause(cause: CashMovementCause): CauseRole {
  switch (cause) {
     ...
     */
    /*
     * App. B.6's stand leases (M7-06) are a rate too, and the difference from
     * `hub_upkeep` below is the whole reason these two classes exist:
     * `commitmentsFor` rebuilds a hub fee from the hubs an airline holds and has
