 ...
 *
 * Two modes share one seat/monument layer:
 *
 *   - **Vector** (the default): a parametric shaded fuselage, wings, engines and
 *     tail drawn from the type's planform. No assets; works for every type.
 *   - **Photo** (when a type has `frame.backdrop`): a real top-down render is the
 *     background, the vector plane/floor/doors/cockpit are suppressed, and the
 *     seats are laid onto the image's own cabin floor (`backdrop.floor`, given as
 *     fractions of the image). Types without art fall back to the vector plane.
 *
 * Either way the seats are placed by the config's real element lengths, so the
 * drawing is the geometry the analysis measures. Colours are token references
 * only; the image is an imported asset URL.
 */
...
import type { NumberedElement } from './layout';
import type { CabinConfig, CabinElement, CabinFrame, MonumentKind } from './types';
import type { CSSProperties, ReactNode } from 'react';

const SCALE = 10; // SVG user units per metre (vector mode)
const SEAT = 13; // seat cell height, units (vector body sizing)
const AISLE = 11; // aisle gap, units (vector body sizing)
const NOSE_M = 5;
const TAIL_M = 6;
const AISLE_UNIT = 0.82; // an aisle gap as a fraction of one seat cell
...
/**
 * Seat-centre offsets and the seat cell size for one row's layout, sized to fill
 * the given body height. Returning the cell lets the seat drawing scale with the
 * cabin — a tall photo floor gets bigger seats than the compact vector body.
 */
function seatRows(layout: string, bodyHeight: number): { offsets: number[]; cell: number } {
  const groups = layout
    .split('-')
  ...
  const units = seats + aisles * AISLE_UNIT;
  const cell = (bodyHeight * 0.92) / Math.max(units, 1);
  const usedHeight = seats * cell + aisles * cell * AISLE_UNIT;
  let cursor = (bodyHeight - usedHeight) / 2 + cell / 2;
 ...
 * lit headrest, armrests down each side, and a soft shadow beneath.
 */
function Seat({ x, cy, w, cell }: { x: number; cy: number; w: number; cell: number }): ReactNode {
  const h = cell * 0.9;
  const top = cy - h / 2;
  const cushionW = w * 0.58;
  const backW = w * 0.34;
  const armH = Math.max(cell * 0.12, 1);
  return (
    <g className="cc-seat">
...
interface Placed {
  entry: NumberedElement;
  x: number;
  w: number;
  element: CabinElement;
}

interface ElementsGeom {
  bodyTop: number;
  bodyBottom: number;
  bodyHeight: number;
  midY: number;
...
/** The seats and monuments, shared by both drawing modes. */
function CabinElements({
  placed,
  geom,
  selectedId,
  onSelect,
  ...
  const { bodyTop, bodyBottom, bodyHeight, midY } = geom;
  return (
    <>
      {placed.map(({ entry, x, w, element }) => {
        ...
        const row = entry.row;
        const { offsets, cell } = seatRows(row.seatLayout, bodyHeight);
        const seatW = Math.min(w * 0.8, cell * 1.4);
        ...
        return (
          ...
          >
            {offsets.map((oy, i) => (
              <Seat key={i} x={seatX} cy={bodyTop + oy} w={seatW} cell={cell} />
            ))}
            {selected && (
              <>
                <rect
...
/**
 * The rendered aeroplane behind the cabin (vector mode) — shaded fuselage skin,
 * wings, engines, winglets and stabilisers from the per-type planform. Hidden
 * from assistive tech.
 */
...
/** Photo mode: a real top-down render with the seats laid on its cabin floor. */
function PhotoCabinMap({
  config,
  frame,
  selectedId,
  onSelect,
  ...
  const bodyLeft = bd.floor.l * bd.w;
  const bodyRight = bd.floor.r * bd.w;
  const bodyTop = bd.floor.t * bd.h;
  const bodyBottom = bd.floor.b * bd.h;
  ...
  let cursor = 0;
  const placed: Placed[] = numbered.map((entry) => {
    const element = entry.kind === 'seats' ? entry.row : entry.monument;
    const lengthM = elementLengthM(element);
    const frac = contentM === 0 ? 0 : cursor / contentM;
    const w = contentM === 0 ? 0 : (lengthM / contentM) * span;
    cursor += lengthM;
    return { entry, x: bodyLeft + frac * span, w, element };
  });

  return (
    <svg
      className="cc-map cc-map--photo"
      viewBox={`0 0 ${String(bd.w)} ${String(bd.h)}`}
      role="group"
      aria-label={`${frame.label} cabin plan`}
      preserveAspectRatio="xMidYMid meet"
    >
      <image href={bd.src} x={0} y={0} width={bd.w} height={bd.h} preserveAspectRatio="none" />
      <CabinElements
        placed={placed}
        geom={{ bodyTop, bodyBottom, bodyHeight, midY }}
...
export function CabinMap({
  ...
  if (frame.backdrop !== undefined) {
    return (
      <PhotoCabinMap config={config} frame={frame} selectedId={selectedId} onSelect={onSelect} />
    );
  }
  ...
  const ticks: number[] = [];
  for (let m = 0; m <= cabinM; m += 10) ticks.push(m);
