...
M3 + M8-15 + M7 facilities
 └─ CARGO freighter & logistics operations          [milestone 46 — COMMITTED 2026-09-07]

M3 + PAX + M6 + PX + LOYALTY-08
 └─ RM revenue management & inventory control       [was MISSING — now milestone 47]
```

### The surface gap a reachability audit opened

```
M4-06 maintenance · M5-06 ground handling · M8-01 P&L
M8-06/07 loans & the default ladder · M8-09 statistics    (all shipped)
 └─ SURFACE player surfaces for shipped systems    [milestone 53]
     └─ SURFACE-01  the audit, and the CI guard  →  CI-SAFE
```

This one is not a missing _domain_. Each subsystem above has an owner, a closed issue and live
endpoints; what none of them had is a consumer in `packages/web/src`. Milestones are organised by
domain and each domain's UI issues sit at its end, so surfaces lag by construction — and where a
domain closed before its surface was filed, the surface was never filed at all. M4 and M5 are
closed, which is why maintenance and ground handling could not go back to them.

The edge that matters is the last one: SURFACE-01's guard is what stops a sixth orphan, and its
home is a CI-SAFE question rather than a SURFACE one.
...
| **AUTH** Multi-Method Authentication       | Discord and Twitch, magic links, passkeys, the identity-linking policy, one session kind, auth auditing                                                                                      | Notification delivery → COMMS                                                                                                                       |
...
| **CARGO** Freighter Operations             | **Committed 2026-09-07.** Freighters, cargo demand and contracts, ULD and capacity, terminals and sorting, network design, conversions — shipped as its own update, after the passenger game | Belly cargo → M8-15 (independent) · Hub cargo facility → CARGO-10 · Cargo stand → M7-06 · Cargo area → M7-07                                        |
...
| **SURFACE** Player Surfaces                | **Player surfaces for subsystems that shipped without one**, and the reachability audit and CI guard that keep the gap from recurring                                                        | Every domain's own logic → that domain · Dashboards → M8-10/11/12/14 · The Board page → M12-02 · Layout systems → UX · The CI gate's home → CI-SAFE |
...
| LEGACY terminal failure (#661)  | **P1**                   | No                            | **M8-07 shipped 2026-09-06 with an administration state and no exit.** No longer prospective                     |
...
| REG traffic rights (#664)       | **P1**                   | No                            | `open-route.ts` passes `hasTrafficRights: true` as a literal. Becomes P0 with ALLIANCE or a historical-era world |
...
| ~~CARGO decision (#667)~~       | **DECIDED 2026-09-07**   | No                            | **Yes — Tailfin has cargo**, as its own update. Recorded on CARGO-01 (#1087); the milestone is the build         |
...
3. **Three gaps were added that the original list did not name**, all from §24 or from the code:
   ...
   (**P1**, measurable at `open-route.ts`'s `hasTrafficRights` literal), and the reset/endgame
   decision (**P0**, §24's own words). Currency and taxation was added at P2.
...
8. #664  REG             — the `hasTrafficRights` literal in `open-route.ts`
9. #661  LEGACY          — **overdue: M8-07 shipped 2026-09-06**
10. #662 SEASON · #663 HISTORY (after #660) · #665 SAFETY
11. ~~#667 CARGO decision~~ — **answered 2026-09-07: yes.** CARGO-01 (#1087) carries it
