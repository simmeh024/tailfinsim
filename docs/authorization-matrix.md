...
| `GET /landing`                                      | Intentionally public marketing surface; fetches nothing (ADR-0028)           | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /landing.css`                                  | Its stylesheet, same-origin so `style-src 'self'` covers it (ADR-0028)       | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /landing-hero.webp`                            | Its backdrop image, same-origin so `img-src 'self'` covers it (ADR-0028)     | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /landing.js`                                   | Its carousel script, same-origin so `script-src 'self'` covers it (ADR-0028) | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /fleet-atr72.webp`                             | Fleet carousel artwork, same-origin (ADR-0028)                               | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /fleet-e190.webp`                              | Fleet carousel artwork, same-origin (ADR-0028)                               | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /fleet-a321neo.webp`                           | Fleet carousel artwork, same-origin (ADR-0028)                               | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /fleet-777.webp`                               | Fleet carousel artwork, same-origin (ADR-0028)                               | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /fleet-747.webp`                               | Fleet carousel artwork, same-origin (ADR-0028)                               | Allow | Allow                                | Allow                                    | Allow                       |
...
| `GET /api/currencies`                               | `requireAuth`; the same currency list and rates for every signed-in player   | 401   | Allow                                | Allow                                    | Allow                       |
| `PUT /api/me/currency`                              | `requireAuth`; writes the session player's own display preference, no id     | 401   | Allow (own account)                  | Allow (own account)                      | Allow (own account)         |
...
| `GET /api/auth/discord`                             | OAuth initiation; signed state + PKCE                                        | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /api/auth/discord/callback`                    | OAuth callback; signed state + PKCE, provider-bound                          | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /api/auth/twitch`                              | OAuth initiation; signed state (no PKCE — Twitch has none)                   | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /api/auth/twitch/callback`                     | OAuth callback; signed state, provider-bound                                 | Allow | Allow                                | Allow                                    | Allow                       |
| `GET /api/auth/google/connect`                      | `requireAuth`; starts a link, intent in the signed cookie                    | 401   | Allow (own account)                  | Allow (own account)                      | Allow (own account)         |
| `GET /api/auth/discord/connect`                     | `requireAuth`; starts a link, intent in the signed cookie                    | 401   | Allow (own account)                  | Allow (own account)                      | Allow (own account)         |
| `GET /api/auth/twitch/connect`                      | `requireAuth`; starts a link, intent in the signed cookie                    | 401   | Allow (own account)                  | Allow (own account)                      | Allow (own account)         |
| `GET /api/me/sign-in-methods`                       | `requireAuth`; the session player's own methods, no id accepted              | 401   | Allow (own account)                  | Allow (own account)                      | Allow (own account)         |
| `DELETE /api/me/sign-in-methods/:identityId`        | `requireAuth`; owner-scoped resolution, 404 for absent or another player's   | 401   | Allow (own account)                  | 404 (cross-owner)                        | 404 (cross-owner)           |
...
| `GET /api/admin/overview`                           | `requireCapability('console.read')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/audit`                              | `requireCapability('audit.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/admins`                             | `requireCapability('admin.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/players`                            | `requireCapability('player.read')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/players/:playerId`                  | `requireCapability('player.read')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/players/:playerId/sessions/revoke` | `requireCapability('player.sessions.revoke')`                                | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/airlines/:airlineId`                | `requireCapability('airline.read')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `PATCH /api/admin/airlines/:airlineId/identity`     | `requireCapability('airline.identity')`                                      | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/worlds/health`                      | `requireCapability('world.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/system-health`                      | `requireCapability('system.read')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/worlds`                             | `requireCapability('world.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/worlds`                            | `requireCapability('world.create')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/worlds/:worldId/speed`             | `requireCapability('world.speed')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/worlds/:worldId/status`            | `requireCapability('world.lifecycle')`                                       | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/worlds/:worldId/reset`             | `requireCapability('world.reset')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/economy-config`                     | `requireCapability('economy.read')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/economy-config/:version`            | `requireCapability('economy.read')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/economy-config/:version/diff`       | `requireCapability('economy.read')`                                          | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/economy-config`                    | `requireCapability('economy.publish')`                                       | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/worlds/:worldId/economy-config`    | `requireCapability('economy.pin')`                                           | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/worlds/:worldId/npc`                | `requireCapability('npc.read')`                                              | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/world-presets`                      | `requireCapability('world.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `GET /api/admin/worlds/:worldId/config`             | `requireCapability('world.read')`                                            | 401   | 403                                  | 403                                      | Allow                       |
| `POST /api/admin/events/requeue`                    | `requireCapability('event.requeue')`                                         | 401   | 403                                  | 403                                      | Allow                       |
...
| `GET /api/world/airports`                           | `requireAuth`; global reference data, no ownership                           | 401   | Allow                                | Allow                                    | Allow                       |
| `GET /api/world/map`                                | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
...
| `GET /api/finance/runway`                           | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/statistics`                               | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/statistics/executive`                     | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/statistics/operations`                    | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/statistics/:metricId/breakdown`           | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/alerts`                                   | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/digest`                                   | `requireAirline`                                                             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/digest/read`                             | `requireAirline`; watermark derived from session, clamped to the world clock | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
...
| `GET /api/crew/roster`                              | `requireAirline`; named crew scoped by resolved owner                        | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/crew/roster/:id/skills`                  | `requireActiveAirline`; member scoped by owner; 404 cross-owner              | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `GET /api/academies`                                | `requireAirline`; academies scoped by resolved owner                         | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/academies`                               | `requireActiveAirline`; base scoped by owner; 404 cross-owner                | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `POST /api/academies/:id/levels`                    | `requireActiveAirline`; academy scoped by owner; 404 cross-owner             | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `POST /api/academies/:id/modules`                   | `requireActiveAirline`; academy scoped by owner; 404 cross-owner             | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
...
| `POST /api/office/executive/hires`                  | `requireActiveAirline`; C-Suite scoped by resolved owner                     | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `DELETE /api/office/executive/hires/:candidateId`   | `requireActiveAirline`; C-Suite scoped by resolved owner                     | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
...
| `GET /api/ground/contracts`                         | `requireAirline`; contracts scoped by resolved owner                         | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
...
| `POST /api/ground/:icao/self-handling`              | `requireActiveAirline`; hub and operation scoped by resolved owner           | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `DELETE /api/ground/self-handling/:id`              | `requireActiveAirline`; operation scoped by resolved owner                   | 401   | 404 for another owner's operation    | Allow when active                        | Same as player/owner        |
| `GET /api/hubs`                                     | `requireAirline`; hubs scoped by resolved owner                              | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/hubs/candidates`                          | `requireAirline`; priced against the resolved owner's hub count              | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/hubs`                                    | `requireActiveAirline`; hub written for the resolved owner                   | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `POST /api/hubs/:hubId/facilities`                  | `requireActiveAirline`; hub scoped by resolved owner                         | 401   | 404 for another owner's hub          | Allow when active                        | Same as player/owner        |
| `GET /api/credit`                                   | `requireAirline`; capacity computed from the resolved owner's own trading    | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/credit/loans`                            | `requireActiveAirline`; loan and cash written under the resolved owner       | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `GET /api/service/catalogue`                        | `requireAirline`; ladders priced by the world's pinned economy               | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/service/packages`                         | `requireAirline`; packages scoped by resolved owner                          | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/service/packages`                        | `requireActiveAirline`; package written under the resolved owner             | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `PUT /api/service/packages/:id`                     | `requireActiveAirline`; package scoped by resolved owner                     | 401   | 404 for another owner's package      | Allow when active                        | Same as player/owner        |
| `DELETE /api/service/packages/:id`                  | `requireActiveAirline`; package scoped by resolved owner                     | 401   | 404 for another owner's package      | Allow when active                        | Same as player/owner        |
| `POST /api/service/payback`                         | `requireAirline`; priced against the resolved owner's own routes             | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `GET /api/service/route-groups`                     | `requireAirline`; groups and routes scoped by resolved owner                 | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/service/route-groups`                    | `requireActiveAirline`; group and its routes scoped by resolved owner        | 401   | 404 for another owner's route        | Allow when active                        | Same as player/owner        |
| `PUT /api/service/route-groups/:id`                 | `requireActiveAirline`; group, routes and package scoped by resolved owner   | 401   | 404 for another owner's group        | Allow when active                        | Same as player/owner        |
| `DELETE /api/service/route-groups/:id`              | `requireActiveAirline`; group scoped by resolved owner                       | 401   | 404 for another owner's group        | Allow when active                        | Same as player/owner        |
...
| `POST /api/airlines`                                | `requireAuth`; founding rules decide eligibility                             | 401   | Allow                                | Allow; one airline per player (ADR-0010) | Allow; no bypass            |
...
| `GET /api/routes/:routeId/performance`              | `requireAirline`; owned route, scoped by resolved owner                      | 401   | 409 without airline; 404 cross-owner | Allow                                    | 404 unless owner; no bypass |
| `GET /api/routes/:routeId/flights`                  | `requireAirline`; owned route, scoped by resolved owner                      | 401   | 409 without airline; 404 cross-owner | Allow                                    | 404 unless owner; no bypass |
| `GET /api/routes/:routeId/diagnosis`                | `requireAirline`; owned route, scoped by resolved owner                      | 401   | 409 without airline; 404 cross-owner | Allow                                    | 404 unless owner; no bypass |
| `GET /api/routes/:routeId/competition`              | `requireAirline`; owned route, scoped by resolved owner                      | 401   | 409 without airline; 404 cross-owner | Allow                                    | 404 unless owner; no bypass |
| `GET /api/routes/:routeId/cargo`                    | `requireAirline`; owned route + owner-scoped `airframeId` query              | 401   | 409 without airline; 404 cross-owner | Allow                                    | 404 unless owner; no bypass |
...
| `DELETE /api/routes/:routeId`                       | `requireOperatingAirline` + owner-scoped delete                              | 401   | 409 without airline; 404 cross-owner | Allow unless ceased                      | 404 unless owner; no bypass |
| `PUT /api/routes/:routeId/active`                   | `requireOperatingAirline` + owner-scoped update                              | 401   | 409 without airline; 404 cross-owner | Allow unless ceased                      | 404 unless owner; no bypass |
| `GET /api/schedules`                                | `requireAirline`; schedules scoped by resolved owner                         | 401   | 409 without an owned airline         | Allow                                    | Same as player/owner        |
| `POST /api/schedules`                               | `requireActiveAirline`; airframe and routes scoped by owner; 404 cross-owner | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
| `PUT /api/schedules/:id`                            | `requireActiveAirline`; schedule and routes scoped by owner; 404 cross-owner | 401   | 409 without an owned airline         | Allow when active                        | Same as player/owner        |
